...
async function readFiles(key: string): Promise<File[]> {
...
async function storeFiles(key: string, files: File[]) {
  const db = await openDatabase();
...
export function useDraftFiles(key: string) {
  ...
  useEffect(() => {
    ...
    void readFiles(key).then(restored => { if (!cancelled && revision.current === initialRevision) { latest.current = restored; updateFiles(restored); setStatus("saved"); } }).catch(() => { if (!cancelled) setStatus("error"); });
  ...
  function setFiles(action: SetStateAction<File[]>) {
    ...
    const pending = (writes.get(key) ?? Promise.resolve()).catch(() => undefined).then(() => storeFiles(key, next));
