...
export function PublicSharePage() {
  ...
  return <div className="public-share"><header><a href="/" className="share-brand"><img src="/brand/yingya-ghost.png" alt=""/><b>映芽</b></a><div className="studio-share-brand"><StudioArtwork variant="workspace"/><span>视频分享</span></div></header><main>
    {loading ? <div className="public-share-state" role="status"><CircleNotch className="spin"/><h1>正在打开视频…</h1></div> : error ? <div className="public-share-state"><StudioArtwork variant="empty"/><h1>暂时无法观看</h1><p role="alert">{error}</p><p>如果链接已过期或取消，请联系分享者获取新的链接。</p><button onClick={() => setRetry(value => value + 1)}><ArrowClockwise/>重新打开</button></div> : video ? <>
