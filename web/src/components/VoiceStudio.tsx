...
export function VoiceStudio({ value, onChange, compact = false }: { value: string; onChange: (voiceId: string) => void; compact?: boolean }) {
  ...
  return <div className={`asset-studio-grid voice-studio ${compact ? "voice-studio--compact" : ""}`}>
    <section className="asset-creator">
      ...
      <div className="asset-mode-tabs" role="tablist" aria-label="音色创建方式"><button role="tab" aria-selected={mode === "design"} className={mode === "design" ? "active" : ""} onClick={() => setMode("design")}><MagicWand/>描述生成</button><button role="tab" aria-selected={mode === "clone"} className={mode === "clone" ? "active" : ""} onClick={() => setMode("clone")}><UploadSimple/>克隆音色</button></div>
