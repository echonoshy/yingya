    ...
    <main className="thread">
      ...
      <footer className="thread-footer">
        ...
        <form className="composer" onSubmit={send} onChange={clearSubmissionFeedback}>
          <textarea aria-label="修改描述" ref={composerRef} value={text} onChange={event => setText(event.target.value)} onKeyDown={event => { if (event.key === "Enter" && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} placeholder={running ? "继续输入，默认排到当前任务之后…" : "描述修改，或继续推进视频…"}/>
