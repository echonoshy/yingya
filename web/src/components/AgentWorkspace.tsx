...
import { ArrowClockwise, ArrowDown, ArrowLeft, ArrowRight, ArrowUp, CaretLeft, Check, CheckCircle, CircleNotch, Code, DownloadSimple, Eye, File, FileAudio, FileText,  FolderSimple, Images, Paperclip, PencilSimple, Plus, Queue, Stop, Terminal, VideoCamera, Warning, X } from "@phosphor-icons/react";
  ...
  return <div className={`workspace workspace--canvas workspace-mobile--${mobilePanel}`} style={workspaceStyle}>
    ...
    <main className="thread">
      ...
      <footer className="thread-footer">
        ...
        <form className="composer" onSubmit={send} onChange={clearSubmissionFeedback}><textarea aria-label="修改描述" ref={composerRef} value={text} onChange={event => setText(event.target.value)} onKeyDown={event => { if (event.key === "Enter" && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} placeholder={running ? "继续输入，默认排到当前任务之后…" : "描述修改，或继续推进视频…"}/><div className="attachment-row">{files.map(file => <span key={file.name}>{file.name}<button type="button" aria-label={`移除 ${file.name}`} onClick={() => setFiles(value => value.filter(item => item !== file))}>×</button></span>)}</div><div className="composer-tools"><div><button className="icon-button" type="button" onClick={() => fileRef.current?.click()} aria-label="添加附件" title="添加附件"><Paperclip/></button><input ref={fileRef} hidden multiple type="file" onChange={event => setFiles(Array.from(event.target.files ?? []))}/><button className={`asset-picker-trigger ${selectedAssets.length ? "active" : ""}`} type="button" aria-expanded={assetPickerOpen} onClick={() => setAssetPickerOpen(open => !open)}><Images/>选择素材{selectedAssets.length ? <span>{selectedAssets.length}</span> : null}</button><VoiceSelector value={project.voiceId} onChange={onVoice} disabled={running}/><ModelSelector models={models} value={selection} onChange={onSelection}/>{running ? <label className="interrupt-toggle"><span>发送方式</span><select aria-label="发送方式" value={interrupt ? "interrupt" : "queue"} onChange={event => setInterrupt(event.target.value === "interrupt")}><option value="queue">排队执行</option><option value="interrupt">中断并立即应用</option></select></label> : null}</div><div>{running ? <button className="stop-button" type="button" disabled={stopping} onClick={() => void stop()}>{stopping ? <CircleNotch className="spin"/> : <Stop weight="fill"/>}{stopping ? "正在停止" : "停止当前任务"}</button> : null}<button className="send-button" aria-label={sendStage === "failed" ? "重试发送消息" : "发送消息"} title={sendStage === "failed" ? "重试发送" : "发送消息"} aria-busy={sendStage === "uploading" || sendStage === "sending"} disabled={(!text.trim() && !files.length && !selectedAssets.length && !feedbackDraft.items.length) || busy || fileDraftStatus === "loading" || feedbackDraft.status === "loading"}>{sendStage === "uploading" || sendStage === "sending" ? <CircleNotch className="spin"/> : sendStage === "failed" ? <ArrowClockwise/> : <ArrowUp weight="bold"/>}</button></div></div></form>{error ? <p className="form-error" role="alert">{error}</p> : null}
      </footer>
    </main>
...
function ArtifactCanvas({ exportRequest, onDescribe, onFeedback, feedbackDisabled, project, activeTab, preview, media, libraryAssets, libraryFolders, selectedAssets, onClosePreview, onTab, onPreview, onContext, onSelectAssets, onTimedFeedback, onCompose, onRefresh }: { exportRequest: number; onDescribe: () => void; onFeedback: (draft: FeedbackDraft) => void; feedbackDisabled: boolean; project: ProjectDetail; activeTab: CanvasTab; preview: { artifact: Artifact; content: string; loading: boolean; error: string } | null; media: AgentMedia; libraryAssets: AssetLibraryItem[]; libraryFolders: AssetFolder[]; selectedAssets: AssetLibraryItem[]; onClosePreview: () => void; onTab: (value: CanvasTab) => void; onPreview: (artifact: Artifact) => void; onContext: (value: string) => void; onSelectAssets: (assets: AssetLibraryItem[]) => void; onCompose: (text: string) => void; onTimedFeedback: (versionLabel: string, feedback: Array<{ time: number; description: string }>) => void; onRefresh: () => Promise<void> }) {
  ...
  return <section className="artifact-canvas" aria-label="作品工作区">
    ...
    {preview ? <InlineArtifactPreview projectId={project.id} preview={preview} onClose={onClosePreview}/> : <>
      {activeTab === "preview" && !videoPath && liveAvailable && exportRequest ? <div ref={exportRef} className="export-destination" role="group" tabIndex={-1} aria-label="导出设置"><p role="status">当前还没有可导出的草稿。制作完成后可在这里导出视频。</p></div> : null}
      ...
      {activeTab === "preview" && (videoPath || !liveAvailable) ? <section className="preview-panel preview-panel--with-inspector"><div className="preview-main">
        <div className="section-heading"><h3>{videoPath ? (finalVideoArtifact ? "已导出视频" : "草稿预览") : "创作方案"}</h3><span>{project.aspectRatio}</span></div>
        ...
        <div ref={exportRef} className="export-destination" role="group" tabIndex={-1} aria-label="导出设置">{version ? <PersistentRenderPanel project={project} version={version} onRefresh={onRefresh}/> : exportRequest ? <p role="status">生成草稿后即可导出视频。请先完成制作方案并生成草稿。</p> : null}</div>
      ...
      {activeTab === "artifacts" ? <section className="artifact-list"><div className="section-heading"><h3>项目文件</h3><span>{project.manifest.artifacts.length}</span></div>{project.manifest.artifacts.map(artifact => <div className="artifact-row" key={artifact.id}><button className="artifact-open" onClick={() => onPreview(artifact)}><span>{artifact.kind.includes("report") ? <Check/> : <File/>}</span><div><b>{artifact.label}</b><small>{artifact.path}</small></div><Eye/></button><button className="artifact-context" aria-label={`加入反馈 ${artifact.label}`} title="加入反馈" onClick={() => onContext(`${version?.label ?? "项目"} · ${artifact.label}`)}>+</button></div>)}{!project.manifest.artifacts.length ? <div className="artifact-empty"><File/><b>还没有生成的文件</b><p>生成完成后，视频与检查报告会显示在这里。</p></div> : null}</section> : null}
    </>}
    </div>
  </section>;
}
