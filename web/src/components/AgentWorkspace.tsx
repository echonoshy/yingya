    ...
    <main className="thread">
      ...
      <footer className="thread-footer">
        ...
        <ComposerForm className="composer" onSubmit={send} onChange={clearSubmissionFeedback} filesDisabled={busy || fileDraftStatus === "loading"} onFiles={added => { clearSubmissionFeedback(); setFiles(current => [...current, ...added]); }}>
          ...
          <div className="composer-tools composer-toolbar"><div className="composer-resources"><button className="icon-button" type="button" onClick={() => fileRef.current?.click()} aria-label="添加附件" title="添加附件"><Paperclip/></button><input ref={fileRef} hidden multiple type="file" onChange={event => { const added = Array.from(event.target.files ?? []); setFiles(current => [...current, ...added]); event.target.value = ""; }}/><button className={`asset-picker-trigger ${selectedAssets.length ? "active" : ""}`} type="button" aria-expanded={assetPickerOpen} onClick={() => setAssetPickerOpen(open => !open)}><Images/>选择素材{selectedAssets.length ? <span>{selectedAssets.length}</span> : null}</button></div>
  ...
  return <section className="artifact-canvas" aria-label="作品工作区">
    ...
    {preview ? <InlineArtifactPreview projectId={project.id} preview={preview} onClose={onClosePreview}/> : <>
      ...
      {activeTab === "preview" && (videoPath || !liveAvailable) ? <section className="preview-panel preview-panel--with-inspector"><div className="preview-main">
        ...
        <div ref={exportRef} className="export-destination" role="group" tabIndex={-1} aria-label="导出设置">{version ? <PersistentRenderPanel project={project} version={version} onRefresh={onRefresh}/> : exportRequest ? <p role="status">请先确认制作方案并完成动画编排，预览满意后在这里导出成片。</p> : null}</div>
