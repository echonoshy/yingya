import { Bell, Check, CircleNotch, Warning, X } from "@phosphor-icons/react";
...
export function TaskCenter({ projects, offline, onOpen }: { projects: ProjectRecord[]; offline: boolean; onOpen: (id: string) => void }) {
  ...
  const pending = projects.filter(project => projectGroup(project) !== "completed");
  return <aside className="task-center" onKeyDown={event => { if (event.key === "Escape") { setOpen(false); event.currentTarget.querySelector<HTMLButtonElement>(".task-center-trigger")?.focus(); } }}>
