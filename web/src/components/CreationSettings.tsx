...
export function creationBrief(settings: CreationSettingsValue) {
  return ([["目标时长", settings.duration], ["目标受众", settings.audience], ["补充风格要求", settings.style], ["字幕", settings.subtitles], ["配乐", settings.music]]).filter(([, value]) => value.trim()).map(([name, value]) => `${name}：${value}`).join("；");
}
